                    ...
                    try {
                      ...
                      if (value.endsWith(".svg") && !value.endsWith("cross.svg")) {
                    	if(jimUtil.isChromeLocal() && jimUtil.chromeExtension){
                    		//use extension
                    		//set svg's relative path in the transfer div
                    		document.getElementById('chromeTransfer').innerText = value;
                    		
                  	    	//send ready event to extension
                  		    var getSVGsEvent = window.document.createEvent('Event');
                  		    getSVGsEvent.initEvent('getSVGsEvent', true, true);  
                  	    	document.getElementById('chromeTransfer').dispatchEvent(getSVGsEvent);
                  	    
                  			$('#chromeTransfer').bind('ajaxSVGsDoneEvent', function () {
                  			  var svg = JSON.parse($(this).text());
                  			  
                      	      parseSVG(svg, $target, args);
                      	      
                      	      document.getElementById('chromeTransfer').innerText=""; // Clear the transfer div
                  		    });
                  			$('#chromeTransfer').bind('ajaxSVGsFailEvent', function () {
                  			  document.getElementById('chromeTransfer').innerText=""; // Clear the transfer div
                  		    });
                  	    } else{
